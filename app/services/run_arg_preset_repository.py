...
class RunArgPresetRepository:
    ...
    def create_preset(payload: RunArgPresetCreate) -> dict:
        with get_db_cursor() as (_, cursor):
            cursor.execute(
                """
                INSERT INTO run_arg_presets (name, workflow_id, workflow_version_id, inputs_json)
                VALUES (%s, %s, %s, %s)
                """,
                (
                    payload.name,
    ...
    def update_preset(preset_id: int, payload: RunArgPresetUpdate) -> dict | None:
        with get_db_cursor() as (_, cursor):
            cursor.execute("SELECT id FROM run_arg_presets WHERE id = %s", (preset_id,))
    ...
    def delete_preset(preset_id: int) -> bool:
        with get_db_cursor() as (_, cursor):
            cursor.execute("DELETE FROM run_arg_presets WHERE id = %s", (preset_id,))
