...
from app.schemas.local_preview import CreateLocalPreviewRequest, LocalPreviewResponse
...
async def create_preview(payload: CreateLocalPreviewRequest, user: dict = Depends(current_user)) -> LocalPreviewResponse:
    ...
    try:
        session = local_previews.create(user_id=user_id, client_id=payload.client_id, workflow_version_id=payload.workflow_version_id, definition=payload.definition, inputs=payload.inputs, target_node_id=payload.target_node_id, connection=connection, confirm_side_effects=payload.confirm_side_effects, keep_browser_open=payload.keep_browser_open)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    delivered = await picker_connections.send_agent(user_id, {"version": 1, "type": "preview.start", "session_id": session.id, "payload": {"run_id": session.run_id, "target_node_id": session.target_node_id, "steps": session.plan, "inputs": session.inputs, "keep_browser_open": session.keep_browser_open, "max_steps": 10_000}})
    if not delivered:
...
async def close_preview(run_id: int, user: dict = Depends(current_user)) -> dict[str, bool]:
    ...
    await picker_connections.send_agent(session.user_id, {"version": 1, "type": "preview.close", "session_id": session.id, "payload": {"run_id": run_id}})
    local_previews.close(run_id, int(user["id"]))
    return {"closed": True}
