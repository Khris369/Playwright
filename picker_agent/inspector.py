...
(() => {
  ...
  const metadata = (element) => {
    ...
    return { tag_name: element.tagName.toLowerCase(), attributes, text: (element.innerText || '').slice(0, 160), role, name: (element.getAttribute('aria-label') || element.getAttribute('title') || element.innerText || '').slice(0, 160), label: null };
  };
    ...
    async def _selected(self, event: dict[str, Any]) -> None:
        ...
        if object_id:
            ...
            result = await self.cdp.send("Runtime.callFunctionOn", {
                "objectId": object_id,
                "returnByValue": True,
                "functionDeclaration": "function () { const e=this; const a={}; for (const x of e.attributes) a[x.name]=x.value; const implicit={BUTTON:'button',A:'link',INPUT:(e.type==='checkbox'?'checkbox':e.type==='radio'?'radio':'textbox'),SELECT:'combobox',TEXTAREA:'textbox'}; return {tag_name:e.tagName.toLowerCase(), attributes:a, text:(e.innerText||'').slice(0,160), role:e.getAttribute('role')||implicit[e.tagName]||null, name:e.getAttribute('aria-label')||e.getAttribute('title')||e.innerText||'', label:null}; }",
            })
